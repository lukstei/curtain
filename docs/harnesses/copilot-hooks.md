---
source: https://code.visualstudio.com/raw/docs/agents/reference/hooks-reference.md
...
This article is the configuration and event-schema reference for hooks in the **Local** harness in VS Code. The Local harness runs in the extension host and uses the PascalCase event names and payloads documented here.
...
> [!NOTE]
> The transcript file format is not a stable hook API and might change between VS Code releases. Use documented event fields such as `tool_name`, `tool_input`, or `prompt` when possible.
